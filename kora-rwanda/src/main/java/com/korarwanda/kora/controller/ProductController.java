...
public class ProductController {
    ...
    @PostMapping("/api/artisan/products/bulk")
    @PreAuthorize("hasAnyRole('ARTISAN', 'ADMIN')")
    @Operation(summary = "Bulk create products with Heritage Tags (Artisan/Admin)")
    public ResponseEntity<ApiResponse<List<ProductDto.Response>>> createBulk(
            @RequestParam Long artisanId,
            @Valid @RequestBody List<ProductDto.Request> requests) {
        return ResponseEntity.ok(ApiResponse.success(
                "Bulk products created", productService.createBulk(artisanId, requests)));
    }
    ...
    @PreAuthorize("hasAnyRole('ARTISAN', 'ADMIN')")
    @Operation(summary = "Delete a product (Artisan/Admin)")

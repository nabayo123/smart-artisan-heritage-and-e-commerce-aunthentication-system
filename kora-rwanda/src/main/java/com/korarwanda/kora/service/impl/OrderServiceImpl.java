...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDto.Response createOrder(Long userId, OrderDto.CreateRequest request) {
        ...
        if (customer != null) {
            emailService.sendOrderConfirmation(customer.getEmail(), customer.getFullName(), saved.getOrderId(), total);
        }
    ...
    public OrderDto.Response updateStatus(Long orderId, OrderStatus status) {
        ...
        if (order.getCustomer() != null) {
            emailService.sendOrderStatusEmail(order.getCustomer().getEmail(), 
                    order.getCustomer().getFullName(), order.getOrderId(), status.name());
        }

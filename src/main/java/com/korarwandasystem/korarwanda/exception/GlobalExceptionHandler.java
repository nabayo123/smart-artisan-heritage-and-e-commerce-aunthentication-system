...
public class GlobalExceptionHandler {
    ...
    // ================================
    @ExceptionHandler({JsonProcessingException.class, org.springframework.http.converter.HttpMessageNotReadableException.class})
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ResponseEntity<Map<String, Object>> handleJsonParseException(Exception ex) {
        Map<String, Object> errors = new HashMap<>();
        errors.put("timestamp", LocalDateTime.now());
        errors.put("status", HttpStatus.BAD_REQUEST.value());
        errors.put("error", "Invalid JSON");
    ...
    public ResponseEntity<Map<String, Object>> handleDataIntegrityViolationException(DataIntegrityViolationException ex) {
        ...
        if (message != null) {
            ...
            if (lowerMessage.contains("users.email") || lowerMessage.contains("duplicate entry") && lowerMessage.contains("email")) {
                ...
                response.put("field", "email");
            } else if (lowerMessage.contains("order_items") && lowerMessage.contains("unique")) {
                response.put("message", "Order item already exists for this order and product combination.");
                response.put("field", "order_item");
            } else {
